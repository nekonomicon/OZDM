...
void DecalGunshot( TraceResult *pTrace, int iBulletType )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_PLAYER_357_SNIPER:// Oz 257 sniper
		default:
			// smoke and decal
...
void CBasePlayerItem::FallThink ( void )
{
	...
	if ( pev->flags & FL_ONGROUND )
	{
		...
		if ( !FNullEnt( pev->owner ) )
		{
			...
			//jammer - removing annoying sound
...
void CBasePlayerItem::AttemptToMaterialize( void )
{
	...
	if ( time == 0 )
	{
		//jammer infinity, timer ran out so spawn but if it is random then spawn something
		//else.
		if(pev->flags == FL_RANDOM)//this will call a rondom selection of weapons
		{
			pev->flags = FL_KILLME;
			SpawnItem(pev->origin.x,pev->origin.y,pev->origin.z,pev->angles.y,W_RANDOM);
		}
		Materialize();
		return;
	}
...
CBaseEntity* CBasePlayerItem::Respawn( void )
{
	...
	//jammer infinity
	if(pev->flags == FL_RANDOM)//the original entity is random
	   pNewWeapon->pev->flags = FL_RANDOM;//now this one is also
...
int CBasePlayerWeapon::UpdateClientData( CBasePlayer *pPlayer )
{
	...
	if ( !pPlayer->m_fWeapon || pPlayer->m_pActiveItem != pPlayer->m_pClientActiveItem || m_iClip != m_iClientClip || state != m_iClientWeaponState || pPlayer->m_iFOV != pPlayer->m_iClientFOV )
	{
...
{
	//jammer infinity, if ammo is random then spawn a different ammo type
	if(pev->flags == FL_RANDOM)
	{
		pev->flags = FL_KILLME;
		SpawnItem(pev->origin.x,pev->origin.y,pev->origin.z,pev->angles.y,A_RANDOM);
	}
