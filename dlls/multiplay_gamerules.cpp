...
BOOL CHalfLifeMultiplay::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pWeapon )
{
	...
	// Oz variables (.value)
	if ( pWeapon->iWeight() > pPlayer->m_pActiveItem->iWeight() && autoswitch.value > 0 && !pPlayer->ctf_hascrowbar)
	{
		return TRUE;
	}
...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
	...
	if ( addDefault )
	{
		...
		if (startshotgun.value > 0)
		{
			pPlayer->GiveNamedItem( "weapon_shotgun" );
		}
		if (startxbow.value > 0)
		{
		...
		pPlayer->GiveAmmo( start9mm.value, "9mm", max9mm.value );
		pPlayer->GiveAmmo( startm203.value, "ARgrenades", maxm203.value );
		pPlayer->GiveAmmo( starturanium.value, "uranium", maxuranium.value );
		pPlayer->GiveAmmo( startxbowbolts.value, "bolts", maxxbowbolts.value );
		pPlayer->GiveAmmo( start357rounds.value, "357", max357rounds.value );
		pPlayer->GiveAmmo( startshells.value, "buckshot", maxshells.value );
		pPlayer->GiveAmmo( startrockets.value, "rockets", maxrockets.value );

		for ( int i = 1; i <= starthgrenades.value; i++ ) {
        pPlayer->GiveNamedItem( "weapon_handgrenade" ); 
		}

		for ( i = 1; i <= starttripmines.value; i++ ) {
			pPlayer->GiveNamedItem( "weapon_tripmine" ); 
		}

		for ( i = 1; i <= startsatchels.value; i++ ) {
			pPlayer->GiveNamedItem( "weapon_satchel" ); 
		}

		for ( i = 1; i <= startsnarks.value; i++ ) {
...
void CHalfLifeMultiplay :: PlayerKilled( CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pInflictor )
{
	...
	pVictim->m_iDeaths += pointsperdeath.value;
	...
	{  // killed self
		pKiller->frags -= pointspersuicide.value;// Oz points
	}
	...
	{  // killed by the world
		pKiller->frags -= pointsperworldkill.value;// Oz points
	}
...
float CHalfLifeMultiplay :: FlWeaponRespawnTime( CBasePlayerItem *pWeapon )
{
	...
	return gpGlobals->time + weaponrespawntime.value;
}
...
{
	return gpGlobals->time + itemrespawntime.value;
}
...
{
	return gpGlobals->time + ammorespawntime.value;
}
...
{
	return healthchargerrecharge.value;
}


float CHalfLifeMultiplay::FlHEVChargerRechargeTime( void )
{
	return suitchargerrecharge.value;
}
...
void CHalfLifeMultiplay :: Oz( CBasePlayer *pl )
{
	// 	CHalfLifeMultiplay::Oz( pl );
	...
	UTIL_HudMessage(pl ,hText, szText);
