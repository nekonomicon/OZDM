...
void CBaseEntity::FireBullets(ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker )
{
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if (iTracerFreq != 0 && (tracerCount++ % iTracerFreq) == 0)
		{
			...
			case BULLET_PLAYER_MP5_SNIPER:// Oz mp5 sniper
		...
		if (tr.flFraction != 1.0)
		{
			...
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damageglock.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_PLAYER_9MM_SNIPER:// Oz glock sniper		
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damageglocksniper.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_PLAYER_MP5:
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damagemp5.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_PLAYER_MP5_SNIPER:// Oz mp5 sniper
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damagemp5sniper.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_PLAYER_BUCKSHOT:	
				 // make distance based!
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damageshotgun.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					// TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damage357.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
			...
			case BULLET_PLAYER_357_SNIPER:// Oz 357 sniper
			// Oz variables (.value)
				pEntity->TraceAttack(pevAttacker, damage357sniper.value, vecDir, &tr, DMG_BULLET); 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
				...
				break;
			case BULLET_PLAYER_9MM_SUPERGLOCK: //jammer new superglock
				pEntity->TraceAttack(pevAttacker,150,vecDir,&tr,DMG_BULLET);//big damage 
				if ( !tracer )
				{
					TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
					DecalGunshot( &tr, iBulletType );
