...
CGrenade *CGrenade::ShootContact( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity )
{
	...
	// Oz variables (.value)
	pGrenade->pev->dmg = damagem203.value;
...
CGrenade * CGrenade:: ShootTimed( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
	...
	// Oz variables (.value)
	//CBasePlayer *pOwner = GetClassPtr((CBasePlayer *)pevOwner);
	if (pevOwner->health <= 0)
		pGrenade->pev->dmg = boobytrapdamage.value;
	else
		pGrenade->pev->dmg = damagehgrenade.value;
...
// Oz flare
CGrenade * CGrenade:: ShootFlare( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
	CGrenade *pGrenade = GetClassPtr( (CGrenade *)NULL );
	pGrenade->Spawn();
	UTIL_SetOrigin( pGrenade->pev, vecStart );
