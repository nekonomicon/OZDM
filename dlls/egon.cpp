...
public:
	...
	//jammer
	void JamEgon(int,int,int,int,BOOL);
...
void CEgon::Spawn( )
{
	...
	// Oz variables (.value)
	m_iDefaultAmmo = givewithegon.value;
...
int CEgon::GetItemInfo(ItemInfo *p)
{
	...
	p->iFlags = 0;
	p->iWeight = weightegon.value;// Oz weight
...
void CEgon::Fire( const Vector &vecOrigSrc, const Vector &vecDir )
{
	...
	case FIRE_NARROW:
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			{
				// Oz variables (.value)
				pEntity->TraceAttack( m_pPlayer->pev, damageegonnarrow.value, vecDir, &tr, DMG_ENERGYBEAM );
			}
	...
	case FIRE_WIDE:
		if ( pev->dmgtime < gpGlobals->time )
		{
			...
			{
				// Oz variables (.value)
				pEntity->TraceAttack( m_pPlayer->pev, damageegonwide.value, vecDir, &tr, DMG_ENERGYBEAM | DMG_ALWAYSGIB);
			}
			ApplyMultiDamage(m_pPlayer->pev, m_pPlayer->pev);
			...
			if ( g_pGameRules->IsMultiplayer() )
			{
				...
				// Oz variables (.value)
				::RadiusDamage( tr.vecEndPos, pev, m_pPlayer->pev, damageegonwide.value/4, 128, CLASS_NONE, DMG_ENERGYBEAM | DMG_BLAST | DMG_ALWAYSGIB );
			}
...
//jammer
void CEgon::JamEgon(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pEgon = CBaseEntity::Create("weapon_egon",pos,angle,ENT(pev));//edict());
  if(random)
	  pEgon->pev->flags = FL_RANDOM;
}

void Egon(int x, int y, int z, int a, BOOL random)
{
  CEgon *pEgon = GetClassPtr((CEgon *)NULL);
  pEgon->JamEgon(x,y,z,a,random);
}
