...
void CCrossbowBolt::BoltTouch( CBaseEntity *pOther )
{
	...
	if (pOther->pev->takedamage)
	{
		...
		{
			// Oz variables (.value)
			pOther->TraceAttack(pevOwner, damagexbow.value, pev->velocity.Normalize(), &tr, DMG_NEVERGIB ); 
		}
		else
		{
			// Oz variables (.value)
			pOther->TraceAttack(pevOwner, damagexbow.value, pev->velocity.Normalize(), &tr, DMG_BULLET | DMG_NEVERGIB ); 
		}
...
void CCrossbowBolt::ExplodeThink( void )
{
	...
	// Oz variables (.value)
	pev->dmg = damagexbowexplode.value;
	iScale = 10;
...
public:
	...
	//jammer
	void JamCrossbow(int,int,int,int,BOOL);
...
void CCrossbow::Spawn( )
{
	...
	// Oz variables (.value)
	m_iDefaultAmmo = givewithxbow.value;
...
int CCrossbow::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo1 = "bolts";
	// Oz variables (.value)
	p->iMaxAmmo1 = maxxbowbolts.value;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	// Oz variables (.value)
	p->iMaxClip = magazinexbow.value;
	p->iSlot = 2;
	p->iPosition = 2;
	p->iId = WEAPON_CROSSBOW;
	p->iFlags = 0;
	// Oz variables (.value)
	p->iWeight = weightxbow.value;// Oz weight
	return 1;
}
...
void CCrossbow::FireSniperBolt()
{
	...
	if (m_pPlayer->ctf_hascycle)
		m_flNextPrimaryAttack = gpGlobals->time + cyclexbow2.value * .5;// Oz rate of fire
	else
		m_flNextPrimaryAttack = gpGlobals->time + cyclexbow2.value;// Oz rate of fire
	...
	if ( tr.pHit->v.takedamage )
	{
		...
		ClearMultiDamage( );
		// Oz variables (.value)
		CBaseEntity::Instance(tr.pHit)->TraceAttack(m_pPlayer->pev, damagexbowsniper.value, vecDir, &tr, DMG_BULLET | DMG_NEVERGIB ); 
...
void CCrossbow::FireBolt()
{
	...
	{
		// Oz variables (.value)
		pBolt->pev->velocity = vecDir * speedxbowwater.value;
		pBolt->pev->speed = speedxbowwater.value;
	}
	else
	{
		// Oz variables (.value)
		pBolt->pev->velocity = vecDir * speedxbowair.value;
		pBolt->pev->speed = speedxbowair.value;
	}
	...
	// Oz runes - haste
	if (m_pPlayer->ctf_hascycle)
		m_flNextPrimaryAttack = gpGlobals->time + cyclexbow.value * .5;// Oz rate of fire
	else
		m_flNextPrimaryAttack = gpGlobals->time + cyclexbow.value;// Oz rate of fire
	...
	// Oz variables (.value)
	m_pPlayer->pev->punchangle.x -= recoilxbow.value;
}
...
void CCrossbow::SecondaryAttack()
{
	...
	{
		// Oz variables (.value)
		m_pPlayer->m_iFOV = zoomamountxbow.value;
		m_fInZoom = 1;
	}
...
void CCrossbow::Reload( void )
{
	...
	// Oz variables (.value)
	if (DefaultReload( magazinexbow.value, CROSSBOW_RELOAD, reloadxbow.value ))
	{
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/xbow_reload1.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0,0xF));
	}
}
...
class CCrossbowAmmo : public CBasePlayerAmmo
{
	...
	{ 
		// Oz variables (.value)
		if (pOther->GiveAmmo( givexbowbolts.value, "bolts", maxxbowbolts.value ) != -1)
		{
...
//jammer
public:
	void JamCrossbowAmmo(int,int,int,int,BOOL);
};
LINK_ENTITY_TO_CLASS( ammo_crossbow, CCrossbowAmmo );

//jammer
void CCrossbow::JamCrossbow(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pCrossbow = CBaseEntity::Create("weapon_crossbow",pos,angle,ENT(pev));
  if(random)
	  pCrossbow->pev->flags = FL_RANDOM;
}

void Crossbow(int x, int y, int z, int a, BOOL random)
{
  CCrossbow *pCrossbow = GetClassPtr((CCrossbow *)NULL);
  pCrossbow->JamCrossbow(x,y,z,a,random);
}

//jammer
void CCrossbowAmmo::JamCrossbowAmmo(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pCrossbowAmmo = CBaseEntity::Create("ammo_crossbow",pos,angle,ENT(pev));
  if(random)
	  pCrossbowAmmo->pev->flags = FL_RANDOM;
}

void CrossbowAmmo(int x, int y, int z, int a, BOOL random)
{
  CCrossbowAmmo *pCrossbowAmmo = GetClassPtr((CCrossbowAmmo *)NULL);
  pCrossbowAmmo->JamCrossbowAmmo(x,y,z,a,random);
}
