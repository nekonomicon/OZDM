...
#define TOTALRUNES 11
#define ALLOWEDRUNES 8
...
void CBasePlayer :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	if ( pev->takedamage )
	{
		...
		case HITGROUP_HEAD:
			// Oz variables (.value)
			flDamage *= headdamage.value;
			break;
		case HITGROUP_CHEST:
			// Oz variables (.value)
			flDamage *= chestdamage.value;
			break;
		case HITGROUP_STOMACH:
			// Oz variables (.value)
			flDamage *= stomachdamage.value;
			break;
		case HITGROUP_LEFTARM:
		case HITGROUP_RIGHTARM:
			// Oz variables (.value)
			flDamage *= armdamage.value;
			break;
		case HITGROUP_LEFTLEG:
		case HITGROUP_RIGHTLEG:
			// Oz variables (.value)
			flDamage *= legdamage.value;
			break;
		default:
			break;
		}
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// oz runes - conversions
	CBasePlayer *pPlayer = GetClassPtr((CBasePlayer *)pev);
	CBasePlayer *pAttack = GetClassPtr((CBasePlayer *)pevAttacker);
	...
	// Oz runes - protection halving
	if (pPlayer->ctf_hasrune && pPlayer->ctf_hasprotect)
		flDamage *= .5;

	//jam: Oz runes - vampire rune
	if(pAttack->IsPlayer() && pAttack->ctf_hasrune && pAttack->ctf_hasvamp && (pPlayer != pAttack))//if u hurt yourself too bad
	{
		if(pAttack->pev->health < maxhealthwithvamprune.value) //max of 200
			pAttack->pev->health += (flDamage / 2); // add it if under 200
		if(pAttack->pev->health > maxhealthwithvamprune.value) //check to make sure we didnt go over 200
			pAttack->pev->health = maxhealthwithvamprune.value; //if so change it back
	}
    ...
    //jam: Oz Runes - testing exploding rune thingy
	if(pev->health <= 0)
	{
		if(pAttack->IsPlayer() && pevAttacker == pPlayer->pev) //if killed self no explosion sorry
			wassuicide = TRUE;
	}
...
void CBasePlayer::WaterMove()
{
	...
	if (pev->waterlevel != 3) 
	{
		...
		// Oz variables (.value)
		pev->air_finished = gpGlobals->time + playerair.value;
		pev->dmg = 2;
	...
	// Oz variables (.value)
	if (!RANDOM_LONG(0,0x1f) && RANDOM_LONG(0,playerair.value-1) >= air)
	{
		switch (RANDOM_LONG(0,3))
			{
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if (ctf_hasredflag)
	{
		...
		gRedFlagDroppedInTransit = true;
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_redflag", 
			              pev->origin,angle, edict() );
		pev->renderfx &= ~kRenderFxGlowShell;			
		

		char szText[128];
		sprintf(szText, "%s lost the Fire flag!\n", STRING(pev->netname));
		CtfMsgToAll(szText);
	}
	if (ctf_hasblueflag)
	{
		ctf_hasblueflag = false;
		Vector angle;
		angle.y = pev->angles.y;
		angle.x = 0;
		angle.z = 0;
		...
		gBlueFlagDroppedInTransit = true;
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_blueflag", 
			              pev->origin,angle, edict() );
		pev->renderfx &= ~kRenderFxGlowShell;			
		
		char szText[128];
		sprintf(szText, "%s lost the Ice flag!\n", STRING(pev->netname));
		CtfMsgToAll(szText);
	}
	
	// Oz runes - on death - respawn
	if (ctf_hasfrag)
	{
		ctf_hasrune = false;
		ctf_hasfrag = false;

		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_frag", pev->origin, pev->angles, edict() );
	...
	if (ctf_hasregen)
	{
		...
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_regen", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	{
		ctf_hasrune = false;
		ctf_hascrowbar = false;

		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_crowbar", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	if (ctf_hascycle)
	{
		...
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_cycle", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	if (ctf_hascloak)
	{
		...
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_cloak", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	if (ctf_hashook)
	{
		...
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_hook", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	{
		ctf_hasrune = false;
		ctf_hasprotect = false;
	
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_protect", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	{
		ctf_hasrune = false;
		ctf_haslowgrav = false;
		pev->gravity = 1;
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_lowgrav", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	if (ctf_hasvamp)
	{
		...
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_vamp", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}

	//jam: Oz Runes - booby trap
	if (ctf_hastrap)
	{
		if(!wassuicide)//if suicide then no explosion
		{
			CGrenade *pGrenade = GetClassPtr((CGrenade *)NULL);
			pev->classname = MAKE_STRING("grenade");
			pev->movetype	= MOVETYPE_NONE;
			pev->solid		= SOLID_NOT;//dont change this
			pev->dmg		= 999;//or this
			pev->nextthink = 0; //or this
			pev->owner = ENT(plr->pev);//??? needs this ?????
			didhaveexploderune = true;
			pGrenade->ShootTimed( plr->pev, Center(), plr->pev->angles, 0);
		}

		ctf_hasrune = false;
		ctf_hastrap = false;

		wassuicide = FALSE; //reset suicide flag
	
		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_trap", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
	...
	//jammer superglock
	if (ctf_hassuperglock)
	{
		ctf_hasrune = false;
		ctf_hassuperglock = false;

		CBaseEntity *pEntity = CBaseEntity::Create( "ctf_superglock", pev->origin, pev->angles, edict() );
		pEntity->pev->velocity.x = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
		pEntity->pev->velocity.z = RANDOM_FLOAT( 0, 300 );
	}
...
//	if (!fAnyButtonDown	&& !( g_pGameRules->IsMultiplayer() && CVAR_GET_FLOAT("mp_forcerespawn") > 0 && (gpGlobals->time > (m_fDeadTime + 5) ) ) )
...
//			ALERT ( at_console, "LongJump!" );
		...
		{// ducking jump
			if (jump > 10)
				pev->velocity.z = sqrt( 2 * 800 * 45.0 * 10); // jump 45 units
			else
				pev->velocity.z = sqrt( 2 * 800 * 45.0 * jump); // jump 45 units
...
void CBasePlayer::PreThink(void)
{
	...
	if ( m_afPhysicsFlags & PFLAG_ONTRAIN )
	{
		...
		// Oz Grapple -> !IsHanging
		if ( m_afButtonPressed & IN_FORWARD && !IsHanging)
		{
			vel = 1;
			pTrain->Use( this, this, USE_SET, (float)vel );
		}
		else if ( m_afButtonPressed & IN_BACK && !IsHanging)
		{
			vel = -1;
			pTrain->Use( this, this, USE_SET, (float)vel );
		}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			{
				// Oz variables (.value)
				if (fallingdamage.value < 1)
					TakeDamage(VARS(eoNullEntity), VARS(eoNullEntity), 0, DMG_FALL ); 
				else
	...
	if (((((int)gpGlobals->time) % 2) == 0) && !donethis && playerid.value > 0)
	{
		donethis = 1;
		Identify();
	}
	...
	// Oz runes - regeneration
	if (ctf_hasrune && ctf_hasregen && (pev->health < maxhealth.value || pev->armorvalue < maxcharge.value))
	{
		regencounter++;
		...
		if (regencounter == 30)
		{
			...
			pev->health += regenrunehealth.value;
			if (pev->health > maxhealth.value)
				pev->health = maxhealth.value;

			pev->armorvalue += regenrunearmor.value;
			if (pev->armorvalue > maxcharge.value)
				pev->armorvalue = maxcharge.value;
		}
	}
}
...
edict_t *EntSelectSpawnPoint( CBaseEntity *pPlayer )
{
	...
	// STRING(*spawn_place)
	char *spawn_place;
	if (pPlayer->ctf_team == 1 && ctf.value)
		spawn_place = "ctf_redspawn";
	else if (pPlayer->ctf_team == 2 && ctf.value)
		spawn_place = "ctf_bluespawn";
	else
		spawn_place = "info_player_deathmatch";
...
// choose a info_player_deathmatch point
	...
	else if ( g_pGameRules->IsDeathmatch() )
	{
		...
		for ( int i = RANDOM_LONG(1,5); i > 0; i-- )
			pSpot = UTIL_FindEntityByClassname( pSpot, spawn_place );
...
void CBasePlayer::Spawn( void )
{
	...
	// Oz variables (.value)
	pev->health			= starthealth.value;
	// Oz variables (.value)
	pev->armorvalue		= startcharge.value;
	pev->takedamage		= DAMAGE_AIM;
	pev->solid			= SOLID_SLIDEBOX;
	pev->movetype		= MOVETYPE_WALK;
	// Oz variables (.value)
	pev->max_health		= maxhealth.value;
	pev->flags			= FL_CLIENT;
	// Oz variables (.value)
	pev->air_finished	= gpGlobals->time + playerair.value;
...
// dont let uninitialized value here hurt the player
	...
	// Oz runes - spawning when first player enters
	ctf_hasrune = false;
	ctf_hasregen = false;
	ctf_hascrowbar = false;
	ctf_hascycle = false;
	...
	if (!g_runes_exist)
	{
		...
		for (int i = 0; i < fragrunenumber.value; i++)
			SpawnRunes("ctf_frag");

		for (i = 0; i < regenrunenumber.value; i++)
			SpawnRunes("ctf_regen");

		for (i = 0; i < crowbarrunenumber.value; i++)
			SpawnRunes("ctf_crowbar");

		for (i = 0; i < hasterunenumber.value; i++)
			SpawnRunes("ctf_cycle");
		
		for (i = 0; i < cloakrunenumber.value; i++)
			SpawnRunes("ctf_cloak");
		
		for (i = 0; i < hookrunenumber.value; i++)
			SpawnRunes("ctf_hook");
		
		for (i = 0; i < protectrunenumber.value; i++)
			SpawnRunes("ctf_protect");
		
		for (i = 0; i < lowgravrunenumber.value; i++)
			SpawnRunes("ctf_lowgrav");
		
		for (i = 0; i < vamprunenumber.value; i++)
		SpawnRunes("ctf_vamp");
		
		for (i = 0; i < traprunenumber.value; i++)
			SpawnRunes("ctf_trap");
		
		for (i = 0; i < superglockrunenumber.value; i++)
			SpawnRunes("ctf_superglock");
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if ((m_flFlashLightTime) && (m_flFlashLightTime <= gpGlobals->time))
	{
		...
		{
			// Oz variables (.value)
			if (m_iFlashBattery && nightops.value < 1)
			{
				m_flFlashLightTime = FLASH_DRAIN_TIME + gpGlobals->time;
				m_iFlashBattery--;
...
void CBasePlayer::MOTD( edict_t *client )
{
...
// Oz player id
void CBasePlayer::Identify( void )
{
	TraceResult tr;
	...
	CBasePlayer *pOther = (CBasePlayer*)CBaseEntity::Instance( tr.pHit );
...
/*	else
		ClientPrint( pev, HUD_PRINTCENTER, "-nobody-\n" );
*/
}

// Oz runes - simplified spawning routines
void CBasePlayer::SpawnRunes(char *szText)
{
 	CBaseEntity *pSpot;
	pSpot = NULL;
	//jammer ctf
	...
	//jammer ctf and dm rune spawning
	if(ctf.value && RedPlayerStarts != 0 && BluePlayerStarts != 0)//ctf so get ctf spawns because may not be dm spawns
	{
	 if(NumPlayerStarts == 0)//no dm spawns so  dont use them
	    z = RANDOM_LONG(1,2);
	 else
		z = 3;//we found some dm even in ctf so use them
	...
	else//not in ctf mode so get dm points only
	{
	   int x = RANDOM_LONG(1,NumPlayerStarts);
	  for(int i = 0; i < x; i++)
		  pSpot = UTIL_FindEntityByClassname( pSpot, "info_player_deathmatch" );
	...
	CBaseEntity *pEntity = CBaseEntity::Create(szText, pSpot->pev->origin, pev->angles, edict() );
