...
public:
	...
	//new jammer
	void JamSpawnPlayerStart(int x, int y ,int z, int a, BOOL random);
...
//new jammer
void SpawnPlayerStart(int x, int y ,int z, int a, BOOL random)
{
 CBaseDMStart *pDMStart = GetClassPtr((CBaseDMStart *)NULL);
 pDMStart->JamSpawnPlayerStart(x,y,z,a,random);
}

//new jammer
void CBaseDMStart::JamSpawnPlayerStart(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pPlayerStart = CBaseEntity::Create("info_player_deathmatch",pos,angle,ENT(pev));
}
