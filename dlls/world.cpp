...
//jammer infinity
#include <io.h>
#define MAXFILES 10 // you cna make this bigger if you want to
BOOL FoundFile;
char DatFileName[128];
#define DAT_PATH_DM "oz\\dm_dat\\"
#define DAT_PATH_CTF "oz\\ctf_dat\\"

//jammer ctf flag follow
...
void CWorld :: Precache( void )
{
	...
	//jammer ctf flag follow
	gBlueFirstFlagSpawn = true;
	gBlueFlagDroppedInTransit = false;
	gRedFirstFlagSpawn = true;
...
// make sure the .NOD file is newer than the .BSP file.
	...
	// Oz CTF - flags at home? yes!
	if (!g_ctf_blueflagathome)
		g_ctf_blueflagathome = true;
	if (!g_ctf_redflagathome)
	...
	if ((infinityctf.value && ctf.value) || (infinity.value && ctf.value == 0))
	{
		/************************************
		DON'T forget to change the path below to whatever
		you want it to be........ do it in all places below
		i know i didnt have to rebuild the path each time
		but it is easier that way :-)
		*******************************************/
		//jammer build file name string
		FoundFile = FALSE; //set to false for first run
		if (ctf.value)
			strcpy(DatFileName,DAT_PATH_CTF);
		else
			strcpy(DatFileName,DAT_PATH_DM);

		strcat(DatFileName,(char *)STRING(gpGlobals->mapname));
		strcat(DatFileName,".dat");
		
		//$$$$$$$$$$$$$$ file random stuff	
		//see if the mapname.dat is there
		if(access(DatFileName,04) == 0)//find the original mapname.dat first
		{
			char IntToAscii[24];
			for(int i = 0, x = 0;i < MAXFILES; i++)
			{
				//just rebuild it each time what the heck
				if (ctf.value)
					strcpy(DatFileName,DAT_PATH_CTF);
				else
					strcpy(DatFileName,DAT_PATH_DM);
				...
				strcat(DatFileName,(char *)STRING(gpGlobals->mapname));
				itoa(i+1,IntToAscii,10);
				strcat(DatFileName,IntToAscii);
				strcat(DatFileName,".dat");
				if(access(DatFileName,04) != 0)
					i = MAXFILES;
				else
					x++;//increment if we find a mapnameX
			}
			if(x == 0)//found nothing but the original mapname.dat
			{
				if (ctf.value)
					strcpy(DatFileName,DAT_PATH_CTF);
				else
			...
			else //found at least one file in sequence
			{
				x = RANDOM_LONG(0,x);

				if(x == 0)//if random_long brings back 0 then go to original mapname.dat
				{
					if (ctf.value)
  						strcpy(DatFileName,DAT_PATH_CTF);
					else
				...
				else//random_long came back with something other than 0 os add it to mapname
				{
					if (ctf.value)
						strcpy(DatFileName,DAT_PATH_CTF);
					else
		...
		//end of random file selection
		//$$$$$$$$ end file random stuff $$$$$$$$$$$$
		
		//jammer look for the file one more time in case
		if(access(DatFileName,04) == 0)
			FoundFile = TRUE;
		else
			FoundFile = FALSE;
