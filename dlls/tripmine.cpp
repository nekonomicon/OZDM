...
void CTripmineGrenade :: MakeBeam( void )
{
	...
	// Oz variables (.value)
	// Oz CTF - team color (needs to be done...)
	m_pBeam->SetColor( colorredtripmine.value, colorgreentripmine.value, colorbluetripmine.value );
...
public:
	...
	//jammer infinity
	void JamTripmine(int,int,int,int,BOOL);
};
...
void CTripmine::Spawn( )
{
   if(FoundFile)
	...
	// Oz variables (.value)
	m_iDefaultAmmo = givewithtripmine.value;
...
int CTripmine::GetItemInfo(ItemInfo *p)
{
	...
	// Oz variables (.value)
	p->iMaxAmmo1 = maxtripmines.value;
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	...
	p->iWeight = weighttripmine.value;// Oz weight
...
//jammer infinity
void CTripmine::JamTripmine(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pTripmine = CBaseEntity::Create("weapon_tripmine",pos,angle,ENT(pev));//edict());
  if(random)
	  pTripmine->pev->flags = FL_RANDOM;
}

void Tripmine(int x, int y, int z, int a, BOOL random)
{
  CTripmine *pTripmine = GetClassPtr((CTripmine *)NULL);
  pTripmine->JamTripmine(x,y,z,a,random);
}
