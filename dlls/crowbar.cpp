...
int CCrowbar::GetItemInfo(ItemInfo *p)
{
	...
	p->iWeight = weightcrowbar.value;// Oz weight
	return 1;
}
...
int CCrowbar::Swing( int fFirst )
{
	...
	if ( tr.flFraction >= 1.0 )
	{
		if (fFirst)
		{
			...
			if (m_pPlayer->ctf_hascycle)
				m_flNextPrimaryAttack = gpGlobals->time + cyclecrowbar.value * .5;// Oz rate of fire
			else
				m_flNextPrimaryAttack = gpGlobals->time + cyclecrowbar.value;// Oz rate of fire
	...
	else
	{
		...
		{
			// Oz runes
			if (m_pPlayer->ctf_hascrowbar)
			// Oz variables (.value)
				pEntity->TraceAttack(m_pPlayer->pev, 1000, gpGlobals->v_forward, &tr, DMG_CLUB ); 
			else
				// first swing does full damage
				// Oz variables (.value)
				pEntity->TraceAttack(m_pPlayer->pev, damagecrowbar.value, gpGlobals->v_forward, &tr, DMG_CLUB ); 
		}
		else
		{
			// Oz runes
			...
			// Oz variables (.value)
			pEntity->TraceAttack(m_pPlayer->pev, damagecrowbar.value / 2, gpGlobals->v_forward, &tr, DMG_CLUB ); 
		}	
		...
		if (m_pPlayer->ctf_hascycle)
		// Oz variables (.value)
			m_flNextPrimaryAttack = gpGlobals->time + cyclecrowbar2.value * .5;// Oz rate of fire
		else
		// Oz variables (.value)
			m_flNextPrimaryAttack = gpGlobals->time + cyclecrowbar2.value;// Oz rate of fire
