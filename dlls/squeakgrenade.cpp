...
void CSqueakGrenade :: Spawn( void )
{
	...
	// Oz variables (.value)
	pev->health			= snarkhealth.value;
	pev->gravity		= 0.5;
	pev->friction		= 0.5;

	// Oz variables (.value)
	pev->dmg = damagesnarkpop.value;

	// Oz variables (.value)
	m_flDie = gpGlobals->time + snarktime.value;
...
void CSqueakGrenade::HuntThink( void )
{
	...
	// Oz variables (.value)
	float flpitch = 155.0 - 60.0 * ((m_flDie - gpGlobals->time) / snarktime.value);
	if (flpitch < 80)
		flpitch = 80;
...
void CSqueakGrenade::SuperBounceTouch( CBaseEntity *pOther )
{
	...
	// higher pitch as squeeker gets closer to detonation time
	// Oz variables (.value)
	flpitch = 155.0 - 60.0 * ((m_flDie - gpGlobals->time) / snarktime.value);
	...
	if ( pOther->pev->takedamage && m_flNextAttack < gpGlobals->time)
	{
		...
		if (tr.pHit == pOther->edict())
		{
			...
			if (tr.pHit->v.modelindex != pev->modelindex)
			{
				...
				ClearMultiDamage( );
				// Oz variables (.value)
				pOther->TraceAttack(pev, damagesnarkbite.value, gpGlobals->v_forward, &tr, DMG_SLASH ); 
				if (m_hOwner != NULL)
				...
				// Oz variables (.value)
				pev->dmg += damagesnarkpop.value; // add more explosion damage
...
public:
	...
	//jammer infinity
	void JamSqueak(int,int,int,int,BOOL);
};
...
void CSqueak::Spawn( )
{
	...
	// Oz variables (.value)
	m_iDefaultAmmo = givewithsnark.value;
...
int CSqueak::GetItemInfo(ItemInfo *p)
{
	...
	p->iWeight = weightsnark.value;// Oz weight
...
//jammer infinity
void CSqueak::JamSqueak(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pSqueak = CBaseEntity::Create("weapon_snark",pos,angle,ENT(pev));
  if(random)
	  pSqueak->pev->flags = FL_RANDOM;
}

void Squeak(int x, int y, int z, int a, BOOL random)
{
  CSqueak *pSqueak = GetClassPtr((CSqueak *)NULL);
  pSqueak->JamSqueak(x,y,z,a,random);
}
