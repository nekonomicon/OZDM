...
void CRecharge::Spawn()
{
	...
	// Oz variables (.value)
	m_iJuice = suitchargercapacity.value;
	pev->frame = 0;			
}
...
{ 
	// Oz variables (.value)
	int maxbattery = maxcharge.value;
	...
	// charge the player
	if (m_hActivator->pev->armorvalue < maxbattery)
	{
		// Oz variables (.value)
		m_iJuice -= suitchargeramount.value;
		
		if (m_iJuice < 0)
			m_iJuice = 0;

		// Oz variables (.value)
		m_hActivator->pev->armorvalue += suitchargeramount.value;

		if (m_hActivator->pev->armorvalue > maxbattery)
			m_hActivator->pev->armorvalue = maxbattery;
	}
