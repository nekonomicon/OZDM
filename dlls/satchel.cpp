...
public:
	...
	//jammer infinity
	void JamSatchel(int,int,int,int,BOOL);
};
...
void CSatchel::Spawn( )
{
	...
	// Oz variables (.value)
	m_iDefaultAmmo = givewithsatchel.value;
...
int CSatchel::GetItemInfo(ItemInfo *p)
{
	...
	p->iWeight = weightsatchel.value;// Oz weight
...
void CSatchel::Throw( void )
{
	if (m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType])
	{
		...
		// Oz variables (.value)
		Vector vecThrow = gpGlobals->v_forward * speedsatchel.value + m_pPlayer->pev->velocity;
...
//jammer infinity
void CSatchel::JamSatchel(int x, int y ,int z, int a, BOOL random)
{
  Vector pos(x,y,z);
  Vector angle(0,a,0);    
  CBaseEntity *pSatchel = CBaseEntity::Create("weapon_satchel",pos,angle,ENT(pev));//edict());
  if(random)
	  pSatchel->pev->flags = FL_RANDOM;
}

void Satchel(int x, int y, int z, int a, BOOL random)
{
  CSatchel *pSatchel = GetClassPtr((CSatchel *)NULL);
  pSatchel->JamSatchel(x,y,z,a,random);
}
